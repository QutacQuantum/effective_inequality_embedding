...
class VarTrotterizedADB:
    """
    A class for implementing the variational version of trotterized adiabtic evolution
    ...
    def __init__(self, article_weight, knapsack_capacity, article_value, optimal, n_layers, 
        ...
        """
        Constructs all the necessary attributes for the VarTrotterizedADB object.
    ...
    ######################################################################
    # define sinosoidal annealing schedule
    ######################################################################
    def schedule(self, time_step, annealing_time):
        """
        Parameters
        ----------
            time_step: float
                the time at a given trotter step (or layer)
            annealing_time: float
                annealing time for trotterized adiabatic evolution

        Returns
        ----------
            s_t : float
                the value of the schedule function at `time_step` for the given `annealing_time`

        """
        s_t = np.sin((np.pi/2) * (np.sin((np.pi*time_step)/(2*annealing_time))**2))**2
        return s_t
    ...
    def U_C(self, qc, gamma):
        """
        Method to create `U_C` for variational TAE (this unitary operator is similar to the phase-separator in QAOA)
    ...
    def U_B(self, qc, beta):
        """
        Method to create `U_B` for variational TAE (this unitary operator is similar to the mixer unitary operator in QAOA) 
    ...
    # Sample from circuit
    ##########################################################################################
    def sample_circuit(self, annealing_time):
        """
        Method to create the complete variational TAE quantum circuit and sample the quantum state for parameterized annealing time

        Parameters
        ----------
            annealing_time : list
                the initial annealing time to create the circuit ansatz
        ...
        annealing_time = annealing_time[0]**2
        dtime = annealing_time/self.n_layers
        for time_step in np.linspace(dtime, annealing_time, self.n_layers):
            s_t = self.schedule(time_step, annealing_time)
            self.U_C(qc, s_t * dtime)
            self.U_B(qc, (1-s_t) * dtime)
    ...
    def qubo_energy_penalty(self, x_val):
        ...
        elif self.qubo_formulation=='standard_qubo': # or self.qubo_formulation=='qaoa_evaluation_with_x':
            energy_capacity = 0
            for i in range(self.number_of_knapsacks):
                temp = sum([self.article_weight[j] * x[i, j] for j in range(self.number_of_articles)])
                num_slack_bits = len(bin(self.knapsack_capacity[i])) - 2
    ...
    ##########################################################################################
    def final_energy_penalty(self, x_val, algo):
        """
        ...
        energy_capacity = 0
        if algo in ['no_slack_qubo', "qaoa_evaluation_with_x"]:
            energy_capacity = 0
            for i in range(self.number_of_knapsacks):
                temp = sum([self.article_weight[j] * x[i, j] for j in range(self.number_of_articles)])
                temp += np.max([0, self.knapsack_capacity[i]-temp])
                temp -= self.knapsack_capacity[i]
                energy_capacity += temp ** 2
        elif algo=='standard_qubo': # or self.qubo_formulation=='qaoa_evaluation_with_x':
            energy_capacity = 0
            for i in range(self.number_of_knapsacks):
                temp = sum([self.article_weight[j] * x[i, j] for j in range(self.number_of_articles)])
                num_slack_bits = len(bin(self.knapsack_capacity[i])) - 2
    ...
    ##########################################################################################
    def objective_function(self, annealing_time):
        """
        ...
        sampled_states = self.sample_circuit(annealing_time)
        all_cost_values = []
        all_bitstrings=[]
        costs = []
        if self.full_state:
    ...
    ##########################################################################################
    # Optimize annealing time using adam optimizer
    ##########################################################################################
    def optimize_annealing_time(self, annealing_time, learning_rate, total_iterations): # 
        """
        Method to optimize the annealing time

        Parameters
        ----------
            annealing_time : list
                the inut annealing time to create the circuit ansatz
            learning_rate : float
        ...
        def callback_method(x):
            ...
            if Nfeval % 1 ==0:
                ...
                # print(f'iteration={Nfeval:d}, objective={objective}, min_val={c_min_val:5f}, params = {x[0]**2}')
        ...
        # Define the gradient
        def grad(annealing_time):
            grad = (func(annealing_time + self.eps)-func(annealing_time - self.eps))/(2*self.eps)
            return grad
    
        # second derivative
        def second_deriv(annealing_time):
            sd = np.zeros(len(annealing_time))
            for j in range(len(annealing_time)):
                sd[j] = (func(annealing_time + self.eps) - 2*func(annealing_time) + func(annealing_time - self.eps))/(self.eps**2)
            return sd
    ...
    ##################################################################################
    def compute_final_metrics(self, sampled_states, algo):
        """
        ...
        for state in sampled_states:
            x_val = list([int(bits) for bits in state])
            (cost, penalty) = self.final_energy_penalty(x_val, algo)
            probability=sampled_states[state]/self.shots
